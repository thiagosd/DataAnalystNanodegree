...
def lineplot_compare(hr_by_team_year_sf_la_csv):
    ...
    hr_by_team_year_sf_la_df = pd.read_csv(hr_by_team_year_sf_la_csv)

    gg = ggplot(hr_by_team_year_sf_la_df, aes('yearID', 'HR', color='teamID')) + geom_point() + geom_line() + ggtitle(
        '# of Home Runs by Year') + xlab('Year') + ylab('# of Home Runs')
