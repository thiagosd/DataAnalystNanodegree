...
#filtered_features = filter(lambda x: x >= 0.2, clf.feature_importances_)
#max_value = max(filtered_features)
#print [k for k, v in enumerate(filtered_features) if v == max_value]

i=0
for v in clf.feature_importances_:
    if v > 0.2:
        print vocab_list[i], v, i
    i += 1
